...
# ロジック名 -> プロセスのコマンドライン中に含まれる部分文字列(マッチング用)。
...
def sample_process(match_substring: str) -> ProcessSample:
    """コマンドラインにmatch_substringを含む全プロセスのRSSを合算してサンプリングする。"""
    total_rss = 0
...
class NodeProcessTracker:
    ...
    def sample(self, elapsed_sec: float) -> None:
        for logical_name, match_substring in self._targets.items():
            sample = sample_process(match_substring)
